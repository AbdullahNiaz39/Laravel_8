...
{
    $result = DB::table('orders_detail')->leftJoin('orders', 'orders.id', '=', 'orders_detail.orders_id')
    ->leftJoin('products_attrs', 'products_attrs.id', '=', 'orders_detail.products_attrs_id')
    ->where(['orders_detail.products_id' => $product_id])
    ->where(['orders_detail.products_attrs_id' => $attr_id])
    ->select('orders_detail.qty','products_attrs.qty as pqty')
    ->get();
return $result;
}
...
function apply_coupon_code($coupon_code)
{
    ...
    return json_encode(['status' => $status, 'msg' => $msg,
        'totalPrice' => $totalPrice, 'coupon_code_val' => $coupon_code_val]);
}
